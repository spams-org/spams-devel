...
possible. It also handles sparse feature matrices for large-scale problems. The following regularizations are implemented:
      \begin{itemize}
         ...
         \item General sum of $\ell_\infty$-norms~(see \cite{mairal10}).
         ...
         \item Mixed $\ell_1/\ell_2$-norms on matrices plus $\ell_1$~\cite{sprechmann}.
         ...
         \item multi-task tree-structured sum of $\ell_\infty$-norms (see \cite{mairal10}).
         \item trace norm.
...
{\footnotesize
\verbatiminput{../src_release/mexProximalGraph.m}
}
...
ensures that $\sum_{i=1}^{p+1}\kappab''(\w)_i= 0$.
   ...
   \begin{displaymath}
    ...
    \end{displaymath}
    This problem can be solved in linear-time~\cite{brucker,hochbaum}
